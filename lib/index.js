 ...
 * @returns {String} — closest match or `null`
 */
Matcher.prototype.multiget = function(q) {
  var vComp = q.replace(/(\d)+(kg|mg|g|mcg|l|ml|\/)+/gi,"").replace(/\s{1,}/g," ").trim().split(/[\s,+\d]+/gi);
  var ret = [];
  var vv;
  var closest = '';

  for(vv in vComp){
    closest = this.get( vComp[vv] );
    ret.push( closest ? closest : null );
  }
